      ...
      <div className="grid grid-cols-1 md:grid-cols-3 border-t border-l border-border-light bg-surface">
        {services.map((service, index) => (
          <motion.div 
            ...
            className="group relative border-r border-b border-border-light p-8 md:p-12 flex flex-col justify-between min-h-[450px] hover:bg-gray-50 transition-colors duration-500 overflow-hidden"
          >
             
             <div className="relative z-10">
                <div className="flex justify-between items-start mb-10">
                  <span className="text-xl font-mono text-text-muted/40 group-hover:text-accent transition-colors duration-500">
                    {service.id}
                  </span>
                  <ArrowUpRight className="w-5 h-5 text-text-muted group-hover:text-accent group-hover:rotate-45 transition-all duration-300" />
                </div>
                
                <h3 className="text-3xl font-serif mb-6 leading-tight group-hover:translate-x-1 transition-transform duration-300">
                  {service.title}
                </h3>
                
                <p className="text-text-secondary text-sm leading-relaxed mb-10 opacity-80">
                  {service.description}
                </p>
             </div>

             <div className="relative z-10 mt-auto">
                <ul className="space-y-4 border-t border-border-light pt-6 group-hover:border-accent/20 transition-colors">
                  {service.features.map((item, idx) => (
                    <li key={idx} className="text-text-primary text-xs font-bold uppercase tracking-wider flex items-center">
                      <span className="w-1 h-1 rounded-full bg-accent mr-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-75"></span>
